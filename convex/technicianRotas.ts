...
// Placeholder technician ID for unassigned clinics - using a valid format that matches Id<"technicians">
const PLACEHOLDER_TECHNICIAN_ID = "m97206fp600rf1nb4pf9mm23d57gfdwq" as unknown as Id<"technicians">;
...
export const generateTechnicianRota = internalMutation({
  args: {
    ...
    selectedClinicIds: v.optional(v.array(v.id("clinics"))), // Added selectedClinicIds
  ...
  handler: async (ctx, args) => {
    ...
    const isTechnicianNotAvailable = (
      ...
      for (const assignment of assignments) {
        if (assignment.technicianId === technician._id) {
          if (doTimesOverlap(assignment.startTime, assignment.endTime, startTime, endTime)) {
            return true;
          }
        }
      }
    ...
    const allActiveClinicsToday = await ctx.db.query("clinics")
      .filter(q => 
    ...
    console.log(`[generateTechnicianRota] Retrieved ${allActiveClinicsToday.length} total active clinics for day ${dayOfWeek} (${dayLabel})`);
    allActiveClinicsToday.forEach(clinic => {
      console.log(`[generateTechnicianRota] Active Clinic Data: ${clinic.name} (ID: ${clinic._id})`, {
        id: clinic._id,
    ...
    // If selectedClinicIds is provided and not empty, only consider those clinics
    // Otherwise, don't consider any clinics (empty array) when no selection is made
    let clinicsToConsider: Doc<"clinics">[] = [];
    
    if (args.selectedClinicIds && args.selectedClinicIds.length > 0) {
      console.log(`[generateTechnicianRota] Filtering clinics based on ${args.selectedClinicIds.length} selected IDs provided:`, args.selectedClinicIds);
      clinicsToConsider = allActiveClinicsToday.filter(clinic => args.selectedClinicIds!.includes(clinic._id));
      console.log(`[generateTechnicianRota] ${clinicsToConsider.length} clinics remain after filtering by selectedClinicIds.`);
    } else {
      console.log(`[generateTechnicianRota] No selectedClinicIds provided or list is empty. No clinics will be included in the rota.`);
      // Set to empty array to ensure no clinics are included
      clinicsToConsider = [];
    }

    // LOGGING: Log all clinics being considered before splitting into warfarin/regular
    console.log(`[generateTechnicianRota] ${clinicsToConsider.length} clinics are being considered for rota generation (post-selection filter):`);
    clinicsToConsider.forEach(clinic => {
      console.log(`[generateTechnicianRota] CONSIDERING CLINIC: ${clinic.name} (ID: ${clinic._id}), Warfarin: ${clinic.requiresWarfarinTraining}`);
    });
    
    // Separate into warfarin and regular clinics FROM clinicsToConsider
    warfarinClinics = clinicsToConsider.filter(clinic => clinic.requiresWarfarinTraining === true);
    regularClinics = clinicsToConsider.filter(clinic => clinic.requiresWarfarinTraining !== true);
    
    console.log(`[generateTechnicianRota] Found ${warfarinClinics.length} active warfarin clinics for ${dayLabel} (after selection filter):`);
    ...
    if (args.includeWarfarinClinics && warfarinClinics.length > 0) {
      ...
      // Log available technicians and their training for diagnostics
      console.log(`[generateTechnicianRota] DIAGNOSTIC: Checking ${availableTechnicians.length} available technicians for Warfarin training:`);
      availableTechnicians.forEach(tech => {
        console.log(`[generateTechnicianRota] DIAGNOSTIC: Tech: ${tech.name} (ID: ${tech._id}), Specialist Training: ${JSON.stringify(tech.specialistTraining)}`);
      });

      for (const clinic of warfarinClinics) {
        console.log(`[generateTechnicianRota] Processing warfarin clinic: ${clinic.name} (ID: ${clinic._id}) at ${clinic.startTime}-${clinic.endTime}, Requires Warfarin Training: ${clinic.requiresWarfarinTraining}`);
        
        // Find technicians with warfarin training (checks array OR boolean flag)
        const warfarinTechnicians = availableTechnicians.filter(t => 
          ...
          t.isWarfarinTrained === true
        );
        console.log(`[generateTechnicianRota] Found ${warfarinTechnicians.length} technicians with Warfarin training after filtering (checked array and boolean).`);
        ...
        if (warfarinTechnicians.length === 0) {
          ...
          // Add a conflict to track that this clinic needs assignment
          conflicts.push({
            type: "unassigned_clinic",
            description: `Warfarin clinic ${clinic.name} (${clinic.startTime}-${clinic.endTime}) needs assignment`,
            severity: "warning"
          });
          
          // Add an assignment with the placeholder technician ID so the clinic appears in the rota
          console.log(`[generateTechnicianRota] PRE-ASSIGN (Warfarin - No Trained): Adding clinic ${clinic.name} with PLACEHOLDER_TECHNICIAN_ID. Current clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
          assignments.push({
            technicianId: PLACEHOLDER_TECHNICIAN_ID,
            type: "clinic",
          ...
          console.log(`[generateTechnicianRota] POST-ASSIGN (Warfarin - No Trained): Clinic ${clinic.name} added. New clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
          console.log(`[generateTechnicianRota] Added placeholder assignment for unassigned warfarin clinic: ${clinic.name}`);
          continue;
        }
        ...
        if (availableWarfarinTechs.length === 0) {
          ...
          // Add an assignment with the placeholder technician ID so the clinic appears in the rota
          console.log(`[generateTechnicianRota] PRE-ASSIGN (Warfarin - No Available Trained): Adding clinic ${clinic.name} with PLACEHOLDER_TECHNICIAN_ID. Current clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
          assignments.push({
            technicianId: PLACEHOLDER_TECHNICIAN_ID,
            type: "clinic",
            location: clinic.name,
          ...
          console.log(`[generateTechnicianRota] POST-ASSIGN (Warfarin - No Available Trained): Clinic ${clinic.name} added. New clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
          console.log(`[generateTechnicianRota] Added placeholder assignment for unassigned warfarin clinic: ${clinic.name}`);
          continue;
        }
        ...
        if (selectedTech) {
          ...
          console.log(`[generateTechnicianRota] PRE-ASSIGN (Warfarin - Assigned): Adding clinic ${clinic.name} with Tech ID: ${selectedTech._id}. Current clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
          assignments.push({
          ...
          console.log(`[generateTechnicianRota] POST-ASSIGN (Warfarin - Assigned): Clinic ${clinic.name} added. New clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
          // Update tracking
          assignedTechnicians.add(selectedTech._id);
        }
      }
      
      // Log all clinic assignments after processing warfarin clinics
      console.log(`[generateTechnicianRota] END OF WARFARIN CLINIC PROCESSING:`);
      const currentClinicAssignments = assignments.filter(a => a.type === 'clinic');
      console.log(`[generateTechnicianRota] Total clinic assignments in rota: ${currentClinicAssignments.length}`);
      currentClinicAssignments.forEach((assignment, index) => {
        const tech = technicians.find(t => t._id === assignment.technicianId);
        const techName = tech ? tech.name : (assignment.technicianId === PLACEHOLDER_TECHNICIAN_ID ? 'PLACEHOLDER' : 'Unknown Technician');
        console.log(`[generateTechnicianRota] Clinic Assignment #${index + 1}: Location: ${assignment.location}, Technician: ${techName} (ID: ${assignment.technicianId}), Time: ${assignment.startTime}-${assignment.endTime}`);
      });
    }
    
    // PART 1B: Next handle regular clinics
    console.log(`[generateTechnicianRota] PASS 1B: Attempting to assign technicians to ${regularClinics.length} regular clinics`);
    
    for (const clinic of regularClinics) {
      console.log(`[generateTechnicianRota] Processing regular clinic: ${clinic.name} (ID: ${clinic._id}) at ${clinic.startTime}-${clinic.endTime}, Requires Warfarin Training: ${clinic.requiresWarfarinTraining}`);
      
      let assignedTechToRegularClinic: Doc<"technicians"> | undefined = undefined;

      // Find technicians who are available and meet specific training requirements for this clinic
      const suitableTechniciansForClinic = availableTechnicians.filter(t => {
        // Skip if technician already has a priority assignment from a previous pass (e.g., Warfarin Clinic)
        if (assignedTechnicians.has(t._id)) {
          return false;
        }
        // Skip if technician is not available during the clinic's time slot (checks general non-availability and clashes with *any* existing assignments)
        if (isTechnicianNotAvailable(t, dayLabel, clinic.startTime, clinic.endTime, assignments)) {
          return false;
        }
        // Check if clinic requires Warfarin training and if the technician has it (checks array OR boolean flag)
        if (clinic.requiresWarfarinTraining) {
          const isTechWarfarinTrained = 
            (t.specialistTraining && Array.isArray(t.specialistTraining) && t.specialistTraining.includes("Warfarin")) ||
            t.isWarfarinTrained === true;
          if (!isTechWarfarinTrained) {
            return false; // Does not have required Warfarin training
          }
        }
        // Potentially add checks for other types of specialist training if clinics can require them
        // e.g., if (clinic.requiresXTraining && (!t.specialistTraining || !t.specialistTraining.includes("XTraining"))) return false;
        
        return true; // Technician is suitable
      });

      if (suitableTechniciansForClinic.length > 0) {
        // Simple selection: pick the first available. Could be randomized or based on other criteria later.
        // To introduce randomness: assignedTechToRegularClinic = shuffleArray(suitableTechniciansForClinic)[0];
        assignedTechToRegularClinic = suitableTechniciansForClinic[0]; 
      }

      if (assignedTechToRegularClinic) {
        console.log(`[generateTechnicianRota] PASS 1B: Assigning ${assignedTechToRegularClinic.name} (ID: ${assignedTechToRegularClinic._id}) to regular clinic ${clinic.name}`);
        assignments.push({
          technicianId: assignedTechToRegularClinic._id,
          type: "clinic",
          location: clinic.name,
          startTime: clinic.startTime,
          endTime: clinic.endTime,
          category: "Clinic" // Generic category for the assignment entry
        });
        assignedTechnicians.add(assignedTechToRegularClinic._id); // Mark technician as assigned in this pass
        console.log(`[generateTechnicianRota] POST-ASSIGN (Regular - Assigned): Clinic ${clinic.name} assigned to ${assignedTechToRegularClinic.name}. New clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
      } else {
        // If no suitable technician is found, assign placeholder and log conflict
        console.log(`[generateTechnicianRota] PRE-ASSIGN (Regular - No Suitable Tech Found): Adding clinic ${clinic.name} with PLACEHOLDER_TECHNICIAN_ID. Current clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
        assignments.push({
          technicianId: PLACEHOLDER_TECHNICIAN_ID,
          type: "clinic",
          location: clinic.name,
        ...
        console.log(`[generateTechnicianRota] POST-ASSIGN (Regular - Placeholder): Clinic ${clinic.name} added with placeholder. New clinic assignments: ${assignments.filter(a => a.type === 'clinic').length}`);
        
        conflicts.push({
          type: "clinic_unassigned",
          description: `Regular clinic ${clinic.name} (${clinic.startTime}-${clinic.endTime}) could not be assigned to a suitable technician.`, // Updated description
          severity: "warning"
        });
        console.log(`[generateTechnicianRota] Added placeholder assignment and conflict for unassigned regular clinic: ${clinic.name}`);
      }
    }
    
    // Log all clinic assignments after processing warfarin and regular clinics
    console.log(`[generateTechnicianRota] END OF ALL CLINIC PROCESSING:`);
    const currentClinicAssignments = assignments.filter(a => a.type === 'clinic');
    console.log(`[generateTechnicianRota] Total clinic assignments in rota: ${currentClinicAssignments.length}`);
    currentClinicAssignments.forEach((assignment, index) => {
      const tech = technicians.find(t => t._id === assignment.technicianId);
    ...
    if (unfilledRequirements.length > 0) {
      ...
      for (const item of prioritizedRequirements) {
        ...
        for (const techInfo of sortedTechnicians) {
          ...
          const reqAssignments = requirementAssignments.get(requirement.name) || [];
          reqAssignments.push(techInfo.technician._id);
          requirementAssignments.set(requirement.name, reqAssignments);
...
export const generateWeeklyRota = mutation({
  args: {
    startDate: v.string(),
    endDate: v.optional(v.string()), // Made endDate optional
    technicianIds: v.array(v.id("technicians")),
    includeWarfarinClinics: v.optional(v.boolean()),
    selectedClinicIds: v.optional(v.array(v.id("clinics"))),
    ignoredUnavailableRules: v.optional(v.object({
      technicianId: v.string(),
      ruleIndices: v.array(v.number())
    })),
    selectedWeekdays: v.optional(v.array(v.string())),
    workingDays: v.optional(v.record(v.string(), v.array(v.string()))), // Map of technicianId to their working days
    additionalRolesByDay: v.optional(v.record(v.string(), v.array(v.object({ roleId: v.string(), roleName: v.string() })))) 
  },
  handler: async (ctx, args) => {
    const assignments: (TechnicianAssignment & { date: string })[] = [];
    const conflicts: Conflict[] = [];
    
    const startDateObj = new Date(args.startDate);
    let endDateObj;

    // Check if a meaningful endDate was provided
    if (args.endDate && args.endDate.trim() !== "") {
      endDateObj = new Date(args.endDate);
    } else {
      // Default endDate to 6 days after startDate if not provided (for a 7-day week)
      endDateObj = new Date(startDateObj);
      endDateObj.setDate(startDateObj.getDate() + 6);
      console.log(`[generateWeeklyRota] endDate was not meaningfully provided (received: '${args.endDate}'). Defaulting to 7-day rota ending on: ${endDateObj.toISOString().split('T')[0]}`);
    }

    // Validate that startDate is before or same as endDate
    if (startDateObj > endDateObj) {
      throw new Error("Invalid date range: startDate must be before or same as endDate");
    }
    ...
    while (currentDate <= endDateObj) {
      ...
      // Skip if the current day is not in selectedWeekdays (if provided)
      if (args.selectedWeekdays && args.selectedWeekdays.length > 0 && !args.selectedWeekdays.includes(dayOfWeek)) {
        console.log(`[generateWeeklyRota] Skipping ${dateString} (${dayOfWeek}) as it's not in selected weekdays.`);
        currentDate.setDate(currentDate.getDate() + 1);
        continue;
      }
      
      console.log(`[generateWeeklyRota] Processing rota for ${dateString} (${dayOfWeek})`);

      // Get additional roles for the current day
      const daySpecificAdditionalRoles = args.additionalRolesByDay?.[dayOfWeek] || [];
      const daySpecificRoleNames = daySpecificAdditionalRoles.map(role => role.roleName);
      ...
      const result = await ctx.runMutation(internal.technicianRotas.generateTechnicianRota, {
        ...
        selectedClinicIds: args.selectedClinicIds, // Pass selectedClinicIds
      ...
      // Add date to each assignment from the daily result
      const assignmentsForDay = result.assignments.map(asm => ({
        ...asm,
        date: dateString // Add the date of the assignment
      }));
      assignments.push(...assignmentsForDay);
...
// Store rota assignments grouped by date into rota documents
export const storeRotaAssignments = mutation({
  args: {
    assignments: v.array(v.any()),
    weekStartDate: v.string()
  },
  handler: async (ctx, args) => {
    const { assignments, weekStartDate } = args;
    console.log(`[storeRotaAssignments] Storing ${assignments.length} assignments for week starting ${weekStartDate}`);
    
    if (assignments.length === 0) {
      console.log(`[storeRotaAssignments] No assignments to store`);
    ...
    assignments.forEach(assignment => {
      ...
      if (!assignmentsByDate[dateString]) {
        assignmentsByDate[dateString] = [];
      }
      
      // Create a copy of the assignment without the date field
      const { date, ...assignmentWithoutDate } = assignment;
      assignmentsByDate[dateString].push(assignmentWithoutDate);
    });
    
    console.log(`[storeRotaAssignments] Assignments grouped by date:`, Object.keys(assignmentsByDate));
    
    // Create a rota document for each date
    const rotaIdsByDate: Record<string, Id<"technicianRotas">> = {};
    
    for (const [dateString, dateAssignments] of Object.entries(assignmentsByDate)) {
      try {
        console.log(`[storeRotaAssignments] Creating rota document for ${dateString} with ${dateAssignments.length} assignments`);
        
        // Check if a draft rota already exists for this date
        const existingRotas = await ctx.db.query("technicianRotas")
          .filter(q => 
            q.and(
        ...
        let rotaId: Id<"technicianRotas">;
        
        if (existingRotas.length > 0) {
          // Update existing rota
          rotaId = existingRotas[0]._id;
          await ctx.db.patch(rotaId, {
            assignments: dateAssignments,
            title: `Technician Rota for ${new Date(dateString).toLocaleDateString()}`
          });
          console.log(`[storeRotaAssignments] Updated existing rota for ${dateString} with ID ${rotaId}`);
        } else {
          // Create new rota
          rotaId = await ctx.db.insert("technicianRotas", {
            date: dateString,
            assignments: dateAssignments,
            status: "draft",
            title: `Technician Rota for ${new Date(dateString).toLocaleDateString()}`,
            includedWeekdays: [], // Empty array as default
            conflicts: [],        // Empty array as default
            staffIds: []          // Empty array as default
          });
          console.log(`[storeRotaAssignments] Created new rota for ${dateString} with ID ${rotaId}`);
        }
        
        rotaIdsByDate[dateString] = rotaId;
      } catch (error) {
        console.error(`[storeRotaAssignments] Error creating rota for ${dateString}:`, error);
      }
    }
    
    console.log(`[storeRotaAssignments] Successfully stored rotas for ${Object.keys(rotaIdsByDate).length} dates`);
    return { rotaIdsByDate };
  }
});
